...
3. **Configure the database connection** in `Db.php`:
   ```php
   $conn = new mysqli('127.0.0.1', 'root', 'your_password', 'bank_db', 3300);
   ```
...
- Click **+ Add Defaulter** to register a new loan defaulter
- Click a row to **view** full client details
- Use the **edit** icon to update a record
- Use the **search bar** to filter by name, account number, status, etc.
- Status updates automatically every time the page loads — no manual input needed

## Loan Types Supported

- Personal Loan, Business Loan, Mortgage
- Agricultural Loan, Education Loan, Emergency Loan

## Status Logic

| Status | Condition |
|--------|-----------|
| `Pending` | Due date is more than 30 days away |
| `Critical` | Due date is within the next 30 days |
